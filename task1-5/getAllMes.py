...
# 提取title + URL + time + article 写入getAllMes.txt
...
def getArt(URL):
    ...
    L = soup.select('article')
    if (len(L) != 0):
        art = (L[0]).text
        time = (soup.select('.css-f1q2g4')[0]).text
        article = re.match(r'.*[0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日', art).group()
    else:
        time = ''
        article = ''
        flag = False
    return article, time, flag
...
with open('getAllMes.txt', 'wb') as fd:
    for url in urls:
        ...
        for x in js['data']['articles']:
            ...
            art, time, flag = getArt(artURL)
            if (flag):
                fd.write((x['title'] + '    ' + artURL +
                          '\n').encode(encoding='utf-8'))
                fd.write((time + '\n' + art + '\n\n').encode(encoding='utf-8'))
            else:
                print('剩下的文章不再爬取，因为DOM文档树发生了改变。')
                sys.exit(0)
